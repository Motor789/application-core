...
class PointCloudUpdater : public UpdaterWithChannelsBase {
 ...
 private:
  ...
  void UpdateTransformMatrixs(
      const std::shared_ptr<apollo::transform::TransformStampeds> &tf_matrix);

  void TransformPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr &point_cloud,
                           const std::string &channel_name);

  std::string GetLidarName(const std::string &channel_name);
  ...
  std::unordered_map<std::string, std::shared_ptr<apollo::transform::Transform>>
      matrixs_;
  std::shared_ptr<apollo::transform::TransformStampeds> pred_matrix_ = nullptr;
};
}  // namespace dreamview
}  // namespace apollo
