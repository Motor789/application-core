...
PointCloudUpdater::PointCloudUpdater(WebSocketHandler *websocket)
  ...
  tf_static_reader_ =
      node_->CreateReader<TransformStampeds>(FLAGS_tf_static_topic);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudUpdater::ConvertPCLPointCloud(
  ...
  tf_static_reader_->Observe();
  auto tf_matrix = tf_static_reader_->GetLatestObserved();
  if (tf_matrix &&
      (!pred_matrix_ || !google::protobuf::util::MessageDifferencer::Equals(
                            *pred_matrix_, *tf_matrix))) {
    UpdateTransformMatrixs(tf_matrix);
    pred_matrix_ = tf_matrix;
  }

  TransformPointCloud(pcl_ptr, channel_name);
...
void PointCloudUpdater::TransformPointCloud(
    ...
    const std::string &channel_name) {
  std::string lidar_name = GetLidarName(channel_name);
  if (lidar_name.empty()) {
    AERROR << "Failed to get lidar name from channel: " << channel_name;
    return;
  } else {
    std::shared_ptr<apollo::transform::Transform> matrix = nullptr;
    // Find transform matrix from tf_matrix
    if (matrixs_.find(lidar_name) != matrixs_.end()) {
      // AINFO << "Found lidar name: " << lidar_name << " matrix";
      matrix = matrixs_[lidar_name];
    }

    if (!matrix) {
      AERROR << "Failed to get matrix from tf_static.";
      return;
    }
    ...
    Eigen::Quaternionf rotation(
        matrix->rotation().qw(), matrix->rotation().qx(),
        matrix->rotation().qy(),
        matrix->rotation()
            .qz());        // Replace with your actual values (w, x, y, z)
...
void PointCloudUpdater::UpdateTransformMatrixs(
    const std::shared_ptr<TransformStampeds> &tf_matrix) {
  for (const auto &transform : tf_matrix->transforms()) {
    matrixs_[transform.child_frame_id()] =
        std::make_shared<apollo::transform::Transform>(transform.transform());
  }
}

std::string PointCloudUpdater::GetLidarName(const std::string &channel_name) {
  ...
  // The first segment is space
  if (segments.size() > 5) {
    return segments[3] + "_" + segments[4];
  }
  return "";
}
